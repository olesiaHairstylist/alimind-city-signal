...
def try_fetch_source(source_id: str):
    ...
    return source, raw_text, str(saved_path)


def fetch_with_fallback():
    source, raw_text, saved_path = try_fetch_source("koeri")

    if raw_text is not None:
        return source, raw_text, saved_path, False

    print("KOERI FAILED → TRY USGS")

    source, raw_text, saved_path = try_fetch_source("usgs")

    return source, raw_text, saved_path, True
...
def run_pipeline():
    ...
    source, raw_text, saved_path, fallback_used = fetch_with_fallback()
